        ...
        {(props.ticker === "") ? <div className="text-center font-weight-bold mt-2">Enter a stock ticker in the search above to see info!</div> : <div className="container">
            <div className="card mt-4">
                ...
                <ul className="list-group list-group-flush card-body">
                    ...
                    <li className={(props.currentPrice >= (Math.sqrt(22.5 * props.bookValue * props.eps))) ? "list-group-item font-weight-bold text-warning" : "list-group-item font-weight-bold text-sucess"}>Graham Number: {isNaN(Math.sqrt(22.5 * props.bookValue * props.eps)) ? "Not Applicable - See above info" : "$" + parseFloat(Math.sqrt(22.5 * props.bookValue * props.eps)).toFixed(2)}</li>
                </ul>
            </div>
            <footer>
